...
function AdvertiseForm() {
  const { locale, setIsAdvertisementModalOpen } = useLocale();
  ...
  return (
    <>
      {!showSuccess && (
        <div className="fixed top-0 left-0 right-0 z-[1000] min-h-screen flex items-center justify-center p-4 bg-[url('https://abp-mahakumbh-hindi.s3.us-east-1.amazonaws.com/overlay-bg.svg')]">
          <div className="w-full max-w-2xl bg-[#fff5e3] rounded-3xl p-8 space-y-6">
            <div className="space-y-2">
              <h1 className="text-[#6a302f] text-4xl md:text-5xl font-medium text-center">
                Advertise with us
              </h1>
              <p className="text-[#7a3837] text-center text-lg">
                Showcase your brand to millions at Mahakumbh & leave a lasting
                impression!
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={!accepted}
                className="w-full bg-[#981111] text-white rounded-lg py-4 text-xl font-medium disabled:opacity-50 transition-colors"
              >
                Get in touch
              </button>
              ...
              <div
                ...
                className="w-full text-center py-2 cursor-pointer text-[#6a302f] text-xl font-medium"
              >
                Close
              </div>
            </form>
      ...
      {showSuccess && <SuccessDialog open={showSuccess} onClose={() => {setIsAdvertisementModalOpen(false)}} />}
    </>
  );
}
